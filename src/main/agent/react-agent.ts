...
function promptRequiresToolUse(prompt: string): boolean {
  ...
  const chineseCommand = /(?:运行|执行|启动|重启|停止|终止|关闭|构建|编译|测试|安装|提交|推送|拉取|下载).{0,20}(?:命令|服务|客户端|应用|进程|项目|测试|构建|依赖|代码|仓库)/i.test(prompt)
  const chineseProcessControl = /(?:帮我|请|现在|直接|立即|重新)?\s*(?:重启|停止|终止|关闭)(?:一下|它|客户端|应用|服务|进程|项目)?/i.test(prompt)
  ...
  return chineseAction || chineseReverseAction || chineseCommand || chineseProcessControl || englishAction || fileMention
}
