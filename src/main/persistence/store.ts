...
export const defaults: AppSettings = { model: { baseUrl: 'https://api.openai.com/v1', apiKey: '', model: 'gpt-4.1-mini', timeoutMs: 300000, maxRetries: 3 }, mcpUrl: '', skillsEnabled: true }
...
export const defaultPolicy: AgentPolicy = {
  systemPrompt: [
    ...
    '你可以读取文件、写入文件和执行命令行，但所有文件操作必须限制在工作区内。',
...
export class LocalStore {
    ...
    try {
      ...
      this.state = {
        ...
        policy: {
          ...defaultPolicy,
          ...draft.policy,
          systemPrompt: draft.policy?.systemPrompt === legacySystemPrompt ? defaultPolicy.systemPrompt : draft.policy?.systemPrompt ?? defaultPolicy.systemPrompt,
          enabledTools: draft.policy?.enabledTools ?? defaultPolicy.enabledTools
        },
  ...
  async addMessage(conversationId: string, message: ChatMessage): Promise<Conversation> {
    ...
    if (message.role === 'user' && conversation.title === '新对话') conversation.title = message.content.slice(0, 28) || '新对话'
  ...
  private normalizeConversations(draft: PersistedStateDraft): Conversation[] {
    if (draft.conversations?.length) return draft.conversations
