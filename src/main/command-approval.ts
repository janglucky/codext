import type { CommandApprovalDetails, CommandApprovalRequest } from '../shared/types'
...
export class CommandApprovalManager {
  ...
  request(target: CommandApprovalTarget, details: CommandApprovalDetails): Promise<boolean> {
    if (target.isDestroyed()) return Promise.resolve(false)
    const id = crypto.randomUUID()
    const request: CommandApprovalRequest = { ...details, id, expiresAt: new Date(Date.now() + this.timeoutMs).toISOString() }
    ...
    return new Promise((resolve) => {
      ...
      try {
        target.send('command:approval-request', request)
      } catch {
        settle(false)
      }
    })
