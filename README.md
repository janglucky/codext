...
- 按当前任务隔离模型上下文：完整会话仅用于本地展示，模型只接收一个相关任务片段；历史附件仅在明确追问时复用，成功任务不再携带旧工具轨迹
...
设置页可以保存多个 OpenAI 兼容模型配置，并指定一个默认模型。聊天输入区底部可以为当前会话选择独立模型；选择“跟随默认模型”时，后续请求会自动使用当前默认配置。模型切换只影响切换后的新任务，不会改变正在运行的任务。
